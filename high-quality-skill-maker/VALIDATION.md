...
Note: this is a structure validator, not a behavior evaluator. It verifies that eval cases exist and are well-formed, but it does not run a model or score whether the Skill chooses the correct mode for a given input. A future `scripts/run_eval_cases.py` can add manual or model-assisted scoring when the project is ready for behavior validation.
